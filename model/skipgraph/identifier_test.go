...
func TestDebugInfo(t *testing.T) {
	...
	// Test CompareGreater
	c := skipgraph.Comparison{skipgraph.CompareGreater, &id2, &id1, uint32(31)}
	...
	// Test CompareLess
	c = skipgraph.Comparison{skipgraph.CompareLess, &id1, &id2, uint32(31)}
	debugInfo = c.DebugInfo()
	...
	// Test CompareEqual
	c = skipgraph.Comparison{skipgraph.CompareEqual, &id1, &id1, uint32(len(id1) - 1)}
	debugInfo = c.DebugInfo()
...
func TestIdentifierCompare(t *testing.T) {
	...
	exp := skipgraph.Comparison{skipgraph.CompareEqual, &id0, &id0, uint32(len(id0) - 1)}
	res := id0.Compare(&id0)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())

	exp = skipgraph.Comparison{skipgraph.CompareEqual, &id1, &id1, uint32(len(id1) - 1)}
	res = id1.Compare(&id1)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())

	exp = skipgraph.Comparison{skipgraph.CompareEqual, &id2, &id2, uint32(len(id2) - 1)}
	res = id2.Compare(&id2)
	require.Equal(t, exp.DebugInfo(), res.DebugInfo())
